...
def main(args):
    ...
    lan = args.lan
    src_text_file = os.path.join(args.data_dir, '{}-en.{}'.format(lan, lan))
    tgt_text_file = os.path.join(args.data_dir, '{}-en.en'.format(lan))
    src_tok_file = os.path.join(args.output_dir, '{}-en.tok.{}'.format(lan, lan))
    tgt_tok_file = os.path.join(args.output_dir, '{}-en.tok.en'.format(lan))
    src_emb_file = os.path.join(args.output_dir, '{}-en.emb.{}'.format(lan, lan))
    tgt_emb_file = os.path.join(args.output_dir, '{}-en.emb.en'.format(lan))

    if os.path.exists(src_emb_file):
        src_all_embeds = np.load(src_emb_file)
    else:
        src_all_embeds = extract_embeddings(
            args, src_text_file, src_tok_file, None, lang=lan, pool_type=args.pool_type)
        np.save(src_emb_file, src_all_embeds)

    if os.path.exists(tgt_emb_file):
        tgt_all_embeds = np.load(tgt_emb_file)
    else:
        tgt_all_embeds = extract_embeddings(
            args, tgt_text_file, tgt_tok_file, None, lang='en', pool_type=args.pool_type)
        np.save(tgt_emb_file, tgt_all_embeds)
